...
const moveProjectCardToInProgress = async ({issue,variables}) =>{
  if (!issue) return
  if (!variables) return
  ...
  async.each(projectCards.edges, async ({ node: {
    ...
    var columnId = currentColumnId
    columnId = "MDEzOlByb2plY3RDb2x1bW45ODQ5MTI3"
    ...
    try {
      console.log(projectCardMutationVariables)
